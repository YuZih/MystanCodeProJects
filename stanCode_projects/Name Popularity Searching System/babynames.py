...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    if name in name_data:                               # existing name
        if year in name_data[name]:                     # existing year
            if int(rank) < int(name_data[name][year]):  # choose higher rank
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            lst = line.split(',')      # [year] & [rank, boy name, girl name]
            if len(lst) == 1:
                year = lst[0].strip()
            else:
                rank = lst[0].strip()
